...
export class TmuxManager {
    ...
    async getSessionWindows(sessionName) {
        try {
            const { stdout } = await execAsync(`tmux list-windows -t "${sessionName}" -F "#{window_index}:#{window_name}:#{window_active}:#{window_panes}:#{window_layout}"`);
            const windows = [];
            for (const line of stdout.trim().split('\n')) {
                if (!line)
                    continue;
    ...
    async captureWindowContent(sessionName, windowIndex, numLines = 50) {
        ...
        try {
            const { stdout } = await execAsync(`tmux capture-pane -t "${sessionName}:${windowIndex}" -p -S -${lines}`);
            return stdout;
        }
        catch (error) {
    ...
    async getWindowInfo(sessionName, windowIndex) {
        try {
            const { stdout } = await execAsync(`tmux display-message -t "${sessionName}:${windowIndex}" -p "#{window_name}:#{window_active}:#{window_panes}:#{window_layout}"`);
            if (stdout.trim()) {
    ...
    async createSession(sessionName, startDirectory) {
        try {
    ...
    async createWindow(sessionName, windowName, startDirectory) {
        try {
            const dirOption = startDirectory ? `-c "${startDirectory}"` : '';
            const { stdout } = await execAsync(`tmux new-window -t "${sessionName}" -n "${windowName}" ${dirOption} -P -F "#{window_index}"`);
    ...
    async findWindowsByName(windowName) {
        const sessions = await this.getSessions();
        const matches = [];
        for (const session of sessions) {
            for (const window of session.windows) {
