...
export class EngineerManager {
  ...
  private projectIdeasPath: string;

  constructor(tmux: TmuxManager, projectIdeasPath: string = '/tmp/toma_project_ideas.json') {
    this.tmux = tmux;
    this.projectIdeasPath = projectIdeasPath;
  }

  async createEngineerSession(config: EngineerSessionConfig): Promise<boolean> {
  ...
  private generateEngineerBriefing(projectPath: string, customBriefing?: string): string {
...
Wait for instructions from the Project Manager before beginning work.`;
  ...
  async sendProjectIdea(sessionName: string, idea: ProjectIdea): Promise<boolean> {
    try {
...
4. Time estimate refinement`;
  ...
  async saveProjectIdea(idea: ProjectIdea): Promise<void> {
    try {
  ...
  async requestStatusUpdate(sessionName: string): Promise<boolean> {
...
Please be specific and include relevant details for project coordination.`;
  ...
  async sendMessageBetweenEngineers(fromSession: string, toSession: string, message: string): Promise<boolean> {
...
This message was sent via the orchestrator system.`;
  ...
  async broadcastMessage(sessions: string[], message: string, excludeSession?: string): Promise<boolean[]> {
...
This message was broadcast to all active engineer sessions.`;
  ...
  async terminateEngineerSession(sessionName: string, reason?: string): Promise<boolean> {
    try {
